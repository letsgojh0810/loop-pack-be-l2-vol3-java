...
### 4.2 앱 재시작 시 DB 초기화 (ddl-auto: create)
...
### 4.3 Redis 캐시 히트 시 500 에러 - 원인 1: 이전 포맷 데이터
...
### 4.4 Redis 캐시 히트 시 500 에러 - 원인 2: `.toList()` 반환 타입
...
### 4.5 Redis 캐시 히트 시 500 에러 - 원인 3: Java record의 `final` 특성
...
**5. `ddl-auto: create` 환경에서의 테스트 워크플로우**
